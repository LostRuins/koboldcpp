...
static void ggml_backend_rpc_buffer_memset_tensor(ggml_backend_buffer_t buffer, ggml_tensor * tensor, uint8_t value, size_t offset, size_t size) {
    ggml_backend_rpc_buffer_context * ctx = (ggml_backend_rpc_buffer_context *)buffer->context;
    rpc_tensor rpc_tensor = serialize_tensor(tensor);
    size_t input_size = sizeof(rpc_tensor) + sizeof(uint64_t) + sizeof(uint64_t) + sizeof(uint8_t) + sizeof(size_t);
    std::vector<uint8_t> input(input_size, 0);
    memcpy(input.data(), &rpc_tensor, sizeof(rpc_tensor));
    memcpy(input.data() + sizeof(rpc_tensor), &offset, sizeof(offset));
    memcpy(input.data() + sizeof(rpc_tensor) + sizeof(offset), &size, sizeof(size));
    memcpy(input.data() + sizeof(rpc_tensor) + sizeof(offset) + sizeof(size), &value, sizeof(value));
    bool status = send_rpc_cmd(ctx->sock, RPC_CMD_BUFFER_CLEAR, input.data(), input.size());
    RPC_STATUS_ASSERT(status);
}
...
namespace {
    std::unordered_map<std::string, ggml_backend_reg_t> g_rpc_reg_map;
    std::mutex g_rpc_reg_mutex;
    uint32_t g_rpc_dev_id = 0;
}

ggml_backend_reg_t ggml_backend_rpc_add_server(const char * endpoint) {
    std::lock_guard<std::mutex> lock(g_rpc_reg_mutex);
